...
hook command는 `WMUX_SURFACE_ID`와 `WMUX_WORKSPACE_ID`가 있을 때만 wmux로 세션
binding IPC를 보낸다. 따라서 wmux 안에서 실행된 로컬 Codex/Claude Code pane만
복원 대상이다. SSH/tmux pane은 로컬 session id/cwd로 오인하지 않도록 제외한다.
